...
# 1. Bounding-Box & Download mit Retry-Loop
bbox = (36.320, 20.116, 38.827, 25.225)  # south, west, north, east
max_retries = 5
base_timeout = 60
for attempt in range(1, max_retries + 1):
    try:
        print(f"Lade OSM-Straßennetz … (Versuch {attempt}, Timeout {base_timeout}s)")
        G = ox.graph_from_bbox(*bbox, network_type="drive", timeout=base_timeout)
        break
    except Exception as e:
        print(f"Fehler beim Laden (Versuch {attempt}): {e}")
        if attempt == max_retries:
            raise
        base_timeout += 60  # Timeout für nächsten Versuch erhöhen
        time.sleep(5)
